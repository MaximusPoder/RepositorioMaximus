 ...
 * @author Elton
 */
public class DAOPescadorComposicao implements IOperationBean<PescadorComposicao> {

    private Conexao conexao;

    public boolean cadastrar(PescadorComposicao bean) {
        String query = "insert     into         PescadorComposicao    " +
    ...
    public boolean excluir(PescadorComposicao bean) {
        try {
            conexao = new Conexao();
            conexao.conecta("mil_interface");
            String query = " delete     from        PescadorComposicao   where  " +
                    "       id = " + bean.getId();
    ...
    public boolean atualizar(PescadorComposicao bean) {
        try {
            conexao = new Conexao();
            conexao.conecta("mil_interface");
            String query = "update         PescadorComposicao     set    " +
    ...
    public List<PescadorComposicao> getListWithQuery(String query) {
        try {
            conexao = new Conexao();
            conexao.conecta("mil_interface");
            ...
            String[] fields = MyUtil.getField(new PescadorComposicao());

            List<PescadorComposicao> es = new ArrayList<PescadorComposicao>();
            while (set.next()) {
                int index = 0;
                PescadorComposicao e = new PescadorComposicao();
    ...
    public PescadorComposicao getObjectWithQuery(String query) {
        try {
            conexao = new Conexao();
            conexao.conecta("mil_interface");
            conexao.execute(query);
            ...
            PescadorComposicao es = null;
            while (set.next()) {
                int index = 0;
                es = new PescadorComposicao();
